...
df = data %>%
  ...
  # We'll use firstrun.dur for the initial analysis
  ...
  rename(psychometric = firstrun.dur) %>%
  dplyr::select(psychometric, ia_text, itemid, ianum, uniform_id,
...
read_merge_data = function(lang) {
  
  rt_data = read.csv( paste("../data/langs/", lang, ".csv", sep=""), header = T, sep = ",") %>%
  dplyr::select(-X, -subid, -trial, -supplementary_id)

  pred_data_short_context = read.csv(paste("../data/results/short_context/mgpt_",lang,"_preds.csv", sep=""), header = T, sep = "\t") %>%
  mutate(ia_idx = ianum) %>%
  ...
  pred_data_long_context = read.csv(paste("../data/results/long_context/mgpt_",lang,"_long_preds.csv", sep=""), header = T, sep = "\t") %>%
  ...
  write.csv(merged_df, paste0("./cleaned_data/", lang, "_clean_data.csv"))
